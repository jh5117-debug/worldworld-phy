...
- `empty_manifest_init`: `PASS` / `PHYS_EDITWORLD_EMPTY_MANIFESTS_ALREADY_PRESENT`
  ...
  - command: `DRY_RUN bash scripts/migration/init_physeditworld_empty_manifests.sh`
- `physeditworld_root_candidates_ranked`: `BLOCKED` / `PHYS_EDITWORLD_ROOT_CANDIDATES_NONE_STRONG`
  ...
  - command: `DRY_RUN bash scripts/migration/rank_physeditworld_root_candidates.sh`
- `physeditworld_root_schema_probe`: `BLOCKED` / `PHYS_EDITWORLD_SCHEMA_PROBE_WAITING_FOR_ROOT`
  ...
  - command: `DRY_RUN bash scripts/migration/probe_physeditworld_root_schema.sh`
- `physeditworld_selected_root_status`: `BLOCKED` / `PHYS_EDITWORLD_ROOT_SELECTION_BLOCKED_NO_ROOT`
  ...
  - command: `DRY_RUN bash scripts/migration/select_physeditworld_root.sh`
- `physeditworld_root_intake`: `BLOCKED` / `PHYS_EDITWORLD_ROOT_INTAKE_WAITING_FOR_EXTERNAL_ROOT`
  ...
  - command: `DRY_RUN bash scripts/migration/prepare_physeditworld_root_intake.sh`
- `locked_handoff_sequence`: `BLOCKED` / `LOCKED_HANDOFF_BLOCKED_AT_ROOT_SCHEMA_PROBE`
  ...
  - command: `DRY_RUN bash scripts/migration/run_physeditworld_locked_handoff_sequence.sh`
- `physeditworld_pai_readiness`: `BLOCKED` / `PHYS_EDIT_WORLD_ROOT_OR_MANIFEST_BLOCKED`
  ...
  - command: `DRY_RUN bash scripts/migration/check_physeditworld_pai_readiness.sh`
- `migration_asset_validation`: `BLOCKED` / `MIGRATION_ASSET_VALIDATION_NAS_BLOCKED`
  ...
  - command: `DRY_RUN bash scripts/migration/validate_physeditworld_migration_assets.sh`
- `approved_copy_manifest_template`: `REVIEW_REQUIRED` / `COPY_PLAN_REVIEW_REQUIRED`
  ...
  - command: `DRY_RUN bash scripts/migration/build_physeditworld_migration_copy_plan.sh`
- `approved_copy_status`: `BLOCKED` / `APPROVED_COPY_BLOCKED_NO_APPROVED_ROWS`
  - evidence: `reports/migration/approved_copy_status.json`
  - command: `DRY_RUN bash scripts/migration/run_approved_migration_copy.sh`
- `pai_handoff_status`: `BLOCKED` / `PAI_HANDOFF_BLOCKED_NAS_OR_ROOT`
  - evidence: `reports/migration/pai_handoff_status.json`
  - command: `DRY_RUN bash scripts/migration/verify_pai_physeditworld_handoff.sh`
- `backend_readiness`: `BLOCKED` / `PHYS_EDITWORLD_BACKEND_BLOCKED_SCAFFOLD_ONLY`
  - evidence: `reports/physeditworld_50h/backend_readiness/backend_readiness.json`
  - command: `DRY_RUN python3 -m cam_physgeo.orchestration.physeditworld_backend_readiness`
- `requirement_matrix`: `BLOCKED` / `PHYS_EDIT_WORLD_PIPELINE_BLOCKED_AT_MIGRATION_READINESS`
  ...
  - command: `DRY_RUN python3 -m cam_physgeo.orchestration.physeditworld_requirement_matrix`
- `pipeline_gate_status`: `BLOCKED` / `PIPELINE_BLOCKED_AT_ROOT_SCHEMA_PROBE`
  ...
  - command: `DRY_RUN python3 -m cam_physgeo.orchestration.physeditworld_pipeline_gate`
