...
def test_checkpoint_eval_blocks_no_visible_gpu_for_real_run(tmp_path: Path, monkeypatch):
    ...
    assert rc == 0
    assert "CHECKPOINT_EVAL_BLOCKED_NO_VISIBLE_GPU" in (tmp_path / "summary.md").read_text(encoding="utf-8")


def test_checkpoint_eval_dry_run_allows_no_visible_gpu(tmp_path: Path, monkeypatch):
    monkeypatch.delenv("CUDA_VISIBLE_DEVICES", raising=False)
    manifest = tmp_path / "eval.jsonl"
    manifest.write_text("{}\n", encoding="utf-8")
    validation = tmp_path / "validation.json"
