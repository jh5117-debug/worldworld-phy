...
if [[ ",${CUDA_VISIBLE_DEVICES:-}," == *",0,"* || "${CUDA_VISIBLE_DEVICES:-}" == "0" || "${CUDA_VISIBLE_DEVICES:-}" == 0,* ]]; then
  echo "Refusing to run full Fast StageA on physical GPU0: CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-}" >&2
  exit 2
fi
export FAST_ONLY=1
...
exec "$PY" -m physical_consistency.stages.stage1_physinone_cam.runner   --config configs/cam_physgeo/fast_stageA_full_high_only.yaml   --branch_mode high_only   "$@"
