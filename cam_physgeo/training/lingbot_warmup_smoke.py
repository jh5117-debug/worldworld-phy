...
def staged_warmup_pilot(args: argparse.Namespace) -> int:
    def stage_log(message: str) -> None:
        ...
        print(f"[staged_warmup_pilot{prefix}] {time.strftime('%Y-%m-%dT%H:%M:%S')} {message}", file=sys.stderr, flush=True)
    ...
    if world_size > 1:
        import torch.distributed as torch_dist  # type: ignore

        if args.device != "cuda":
            raise RuntimeError("distributed staged_warmup_pilot requires --device cuda")
        if not torch.cuda.is_available():
            raise RuntimeError("distributed staged_warmup_pilot requires CUDA")
        torch.cuda.set_device(local_rank)
        if not torch_dist.is_initialized():
            init_kwargs: dict[str, Any] = {
                "backend": "nccl",
                "init_method": "env://",
                "timeout": _dt.timedelta(seconds=max(600, int(getattr(args, "ddp_timeout_sec", 1800) or 1800))),
            }
            try:
                init_kwargs["device_id"] = torch.device(f"cuda:{local_rank}")
                torch_dist.init_process_group(**init_kwargs)
            except TypeError:
        ...
        stage_log(f"distributed_ready world_size={world_size} local_rank={local_rank} device={args.device}")

    is_rank0 = rank == 0

    def dist_barrier() -> None:
        if distributed_enabled and dist is not None:
            dist.barrier()

    def dist_destroy() -> None:
        if distributed_enabled and dist is not None and dist.is_initialized():
            dist.destroy_process_group()

    def dist_reduce_mean_scalar(value: float) -> float:
        if not distributed_enabled or dist is None:
            return float(value)
        tensor = torch.tensor([float(value)], device=_torch_device(args.device), dtype=torch.float32)
        dist.all_reduce(tensor, op=dist.ReduceOp.SUM)
        tensor /= float(world_size)
        return float(tensor.item())

    def dist_any_true(value: bool) -> bool:
        if not distributed_enabled or dist is None:
            return bool(value)
        tensor = torch.tensor([1 if value else 0], device=_torch_device(args.device), dtype=torch.int32)
        dist.all_reduce(tensor, op=dist.ReduceOp.MAX)
        return bool(int(tensor.item()))

    def dist_sync_gradients(params: list[Any]) -> None:
        if not distributed_enabled or dist is None:
            return
        for param in params:
    ...
    payload["distributed"] = {
        ...
        "backend": "nccl" if distributed_enabled else None,
    }
    out_dir = Path(args.out)
    ...
    rank_train_metrics_path = metrics_dir / f"train_metrics_rank{rank}.jsonl"
    rank_val_metrics_path = metrics_dir / f"val_metrics_rank{rank}.jsonl"
    ...
    if is_rank0:
        for path in [train_metrics_path, val_metrics_path]:
            if path.exists():
                path.unlink()
    for path in [rank_train_metrics_path, rank_val_metrics_path]:
        if path.exists():
            path.unlink()
    dist_barrier()
    ...
    try:
        ...
        sample_plan, sampler_summary = _make_sample_plan(
            train_rows,
            max_steps=int(args.max_steps) * max(1, world_size),
            sampler=args.sampler,
            balance_keys=_split_csv(args.balance_keys),
            shuffle_seed=int(args.shuffle_seed),
            sample_without_replacement=_bool_arg(args.sample_without_replacement),
        ...
        val_plan, val_sampler_summary = _make_sample_plan(
            val_rows,
            max_steps=val_event_count * max(1, world_size),
            sampler=args.sampler,
            balance_keys=_split_csv(args.balance_keys),
        ...
        summary: dict[str, Any] = {
            ...
            "global_batch_size": int(args.batch_size) * max(1, world_size),
            "distributed": {
                "enabled": distributed_enabled,
                "world_size": world_size,
                "backend": "nccl" if distributed_enabled else None,
                "launcher": "torchrun/env" if distributed_enabled else "single_process",
                "gradient_sync": "manual_all_reduce_mean_lora_grads" if distributed_enabled else None,
                "rank0_writes_metrics_and_checkpoints": True if distributed_enabled else None,
            },
        ...
        def compute_sample_loss(row: dict[str, Any], *, step: int, enable_grad: bool, split: str):
            ...
            rank_suffix = f"_rank{rank}" if distributed_enabled else ""
            cond_dir = out_dir / ("train_logs" if split == "train" else "val_logs") / f"{split}_step_{step:05d}{rank_suffix}"
            forward_condition = adapter._build_forward_condition(
                pair=pair,
                pipe=pipe,
                latent_shape=list(latent.shape),
        ...
        for step in range(1, int(args.max_steps) + 1):
            ...
            row_idx = ((step - 1) * max(1, world_size) + rank) % len(sample_plan)
            row = sample_plan[row_idx]
            optimizer.zero_grad(set_to_none=True)
            loss, metric = compute_sample_loss(row, step=step, enable_grad=True, split="train")
            local_loss_finite = bool(torch.isfinite(loss.detach()).cpu().item())
            any_non_finite_loss = dist_any_true(not local_loss_finite)
            metric["loss_rank_finite"] = local_loss_finite
            metric["any_non_finite_loss"] = any_non_finite_loss
            metric["distributed_rank"] = rank
            metric["distributed_world_size"] = world_size
            metric["global_batch_size"] = int(args.batch_size) * max(1, world_size)
            if any_non_finite_loss:
                skipped_batches += 1
                skip_reason = {
                    "step": int(step),
                    ...
                    "camera_variant": row.get("camera_variant"),
                    "rank": int(rank),
                    "local_loss_finite": local_loss_finite,
                }
                skipped_reasons.append(skip_reason)
                metric["skipped_batch"] = True
                metric["skip_reason"] = skip_reason["reason"]
                _append_jsonl(rank_train_metrics_path, metric)
                if is_rank0:
                    _append_jsonl(train_metrics_path, metric)
                del loss
                optimizer.zero_grad(set_to_none=True)
                _cleanup_cuda()
                if (not _bool_arg(args.skip_non_finite_batches)) or skipped_batches > int(args.max_skipped_batches):
                    stop_reason = f"non_finite_train_loss_step_{step}"
                    break
                continue
            loss.backward()
            dist_sync_gradients(optimizer_params)
            ...
            any_non_finite_grad = dist_any_true(bool(grad_state["any_nan_grad"] or grad_state["any_inf_grad"]))
            if any_non_finite_grad:
                skipped_batches += 1
                skip_reason = {
                    "step": int(step),
                    ...
                    "camera_variant": row.get("camera_variant"),
                    "rank": int(rank),
                    "local_any_nan_grad": bool(grad_state["any_nan_grad"]),
                    "local_any_inf_grad": bool(grad_state["any_inf_grad"]),
                }
                skipped_reasons.append(skip_reason)
                metric["skipped_batch"] = True
                metric["skip_reason"] = skip_reason["reason"]
            ...
            local_loss_value = float(metric["loss"])
            global_loss_value = dist_reduce_mean_scalar(local_loss_value)
            metric["loss_rank"] = local_loss_value
            metric["loss"] = global_loss_value
            metric["distributed_rank"] = rank
            metric["distributed_world_size"] = world_size
            metric["global_batch_size"] = int(args.batch_size) * max(1, world_size)
            train_losses.append(global_loss_value)
            ...
            if step % val_every == 0 or step == int(args.max_steps):
                with torch.no_grad():
                    val_event_idx = max(0, (step // val_every) - 1)
                    val_idx = max(0, min(len(val_plan) - 1, val_event_idx * max(1, world_size) + rank))
                    ...
                    val_local_loss = float(val_metric["loss"])
                    val_global_loss = dist_reduce_mean_scalar(val_local_loss)
                    val_metric["loss_rank"] = val_local_loss
                    val_metric["loss"] = val_global_loss
                    val_metric["distributed_rank"] = rank
                    val_metric["distributed_world_size"] = world_size
                    _append_jsonl(rank_val_metrics_path, val_metric)
                    if is_rank0:
                        _append_jsonl(val_metrics_path, val_metric)
                    val_losses.append(val_global_loss)
                    if dist_any_true(not bool(val_metric["loss_finite"])):
        ...
        if save_adapter_checkpoint and stop_reason is None and steps_completed >= 20 and is_rank0:
        ...
        success = bool(
            ...
            and (not save_adapter_checkpoint or not is_rank0 or checkpoint_info.get("saved") is True)
            and stop_reason is None
        )
        summary.update(
            {
                ...
                "skip_non_finite_batches": _bool_arg(args.skip_non_finite_batches),
                "max_skipped_batches": int(args.max_skipped_batches),
...
def main() -> int:
    ...
    parser.add_argument("--ddp_timeout_sec", type=int, default=1800)
    parser.add_argument("--skip_non_finite_batches", default="true")
    parser.add_argument("--max_skipped_batches", type=int, default=20)
