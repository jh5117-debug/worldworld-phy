...
def main(argv=None):
    ap=argparse.ArgumentParser(); ap.add_argument('--config',required=True); ap.add_argument('--pair_manifest',default=''); ap.add_argument('--dry-run',action='store_true'); ap.add_argument('--run',action='store_true'); ap.add_argument('--limit_pairs',type=int,default=0); ap.add_argument('--max_steps',type=int,default=0); a=ap.parse_args(argv)
    ...
    print_dry_run_plan('stage2_anchored_dpo', cfg, pair_manifest=pair_manifest, pairs=dataset_len, first_pair=first_pair, limit_pairs=a.limit_pairs, max_steps=a.max_steps, real_model_adapter='LingBot branch loading via cam_physgeo.training.model_loading; DPO optimizer guarded behind --run')
