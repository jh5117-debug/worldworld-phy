...
def main() -> None:
    ...
    for row in rows:
        ...
        result = evaluate_csgc_video(video, poses, intr, frame_i=args.frame_i, frame_j=args.frame_j, pose_convention=args.pose_convention, visualization_path=vis)
        out_rows.append({**row, **result.to_dict(), "visualization": str(vis) if vis else ""})
