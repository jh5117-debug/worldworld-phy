...
def semantic_splits(s: dict) -> list[str]:
    ...
    return out
def main(argv=None):
    ap=argparse.ArgumentParser(); ap.add_argument('--manifest',required=True); ap.add_argument('--out_dir',default='manifests'); ap.add_argument('--val-ratio',type=float,default=0.05); ap.add_argument('--test-ratio',type=float,default=0.05); ap.add_argument('--dry-run',action='store_true'); a=ap.parse_args(argv)
    ...
    if not a.dry_run:
        out=Path(a.out_dir)
        for k,v in buckets.items(): write_jsonl(v,out/f'cam_physgeo_{k}.jsonl')
    return 0
